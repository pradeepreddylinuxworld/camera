...
static int cam_smmu_map_buffer_and_add_to_list(int idx, int ion_fd,
	struct dma_buf *buf , struct sg_table *table, enum dma_data_direction dma_dir, dma_addr_t *paddr_ptr,
	size_t *len_ptr);
...
int cam_smmu_get_phy_addr(int handle, int ion_fd, struct dma_buf *buf,
		struct sg_table *table, enum cam_smmu_map_dir dir, dma_addr_t *paddr_ptr,
		size_t *len_ptr)
{
	int idx, rc;
	...
	*len_ptr = (size_t)0;
	...
	rc = cam_smmu_map_buffer_and_add_to_list(idx, ion_fd, buf, table, dma_dir,
			paddr_ptr, len_ptr);
	if (rc < 0) {
		pr_err("Error: mapping or add list fail\n");
		goto get_addr_end;
