...
#include <../drivers/media/platform/qcom/camera/sensor/cci/msm_cci.h>
...
static int msm_camera_pinctrl_init(struct ov5645 *ov5645)
{
	struct pinctrl_info *sensor_pctrl = NULL;

	sensor_pctrl = &ov5645->pinctrl_info;
	sensor_pctrl->pinctrl = devm_pinctrl_get(ov5645->dev);
...
static int ov5645_set_power_on(struct ov5645 *ov5645)
{
	...
	ret = msm_camera_pinctrl_init(ov5645);
	if (ret < 0) {
		pr_err("%s:%d Initialization of pinctrl failed\n",
				__func__, __LINE__);
		ov5645->cam_pinctrl_status = 0;
	} else {
		ov5645->cam_pinctrl_status = 1;
	}


	if (ov5645->cam_pinctrl_status) {
		ret = pinctrl_select_state(ov5645->pinctrl_info.pinctrl,
			ov5645->pinctrl_info.gpio_state_active);
		if (ret)
			pr_err("%s:%d cannot set pin to active state",
				__func__, __LINE__);
	}

	ret = clk_prepare_enable(ov5645->cam_src_clk);
	if (ret < 0) {
		dev_err(ov5645->dev, "cam_src_clk prepare enable failed\n");
	...
	if (ret < 0) {
		dev_err(ov5645->dev, "xclk prepare enable failed\n");
		ov5645_regulators_disable(ov5645);
		return ret;
	}
...
static void ov5645_set_power_off(struct ov5645 *ov5645)
{
	...
	if (ov5645->cam_pinctrl_status) {
		ret = pinctrl_select_state(ov5645->pinctrl_info.pinctrl,
				ov5645->pinctrl_info.gpio_state_suspend);
		if (ret)
			pr_err("%s:%d cannot set pin to suspend state",
				__func__, __LINE__);
		devm_pinctrl_put(ov5645->pinctrl_info.pinctrl);
		ov5645->cam_pinctrl_status = 0;
	}
	ov5645_regulators_disable(ov5645);
	pr_err("%s-\n", __func__);
...
static int ov5645_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (ov5645->power_count == !on) {
		if (on) {
			...
			if (ret < 0) {
				dev_err(ov5645->dev,
					"could not set sys control registers\n");
				ov5645_set_power_off(ov5645);
				goto exit;
			}
		} else {
			     ...
			     struct v4l2_subdev_format *format)
{
	...
	format->format = *__ov5645_get_pad_format(ov5645, fh, format->pad,
						  format->which);
	return 0;
}

static struct v4l2_rect *
__ov5645_get_pad_crop(struct ov5645 *ov5645, struct v4l2_subdev_fh *fh,
			     ...
			     struct v4l2_subdev_format *format)
{
	...
	__crop = __ov5645_get_pad_crop(ov5645, fh, format->pad,
			format->which);
	...
	__format = __ov5645_get_pad_format(ov5645, fh, format->pad,
			format->which);
				  ...
				  struct v4l2_subdev_fh *fh)
{
	...
	fmt.which = fh ? V4L2_SUBDEV_FORMAT_TRY : V4L2_SUBDEV_FORMAT_ACTIVE;
	fmt.format.width = 1920;
	fmt.format.height = 1080;

	ov5645_set_format(subdev, fh, &fmt);
			   ...
			   struct v4l2_subdev_selection *sel)
{
	...
	sel->r = *__ov5645_get_pad_crop(ov5645, fh, sel->pad,
					sel->which);
	return 0;
}
			...
			const struct i2c_device_id *id)
{
	...
	ov5645->cam_src_clk = devm_clk_get(dev, "cam_src_clk");
	if (IS_ERR(ov5645->cam_src_clk)) {
		dev_err(dev, "could not get cam_src_clk");
		return PTR_ERR(ov5645->cam_src_clk);
	}

	cam_src_clk_freq = clk_get_rate(ov5645->cam_src_clk);
	if (cam_src_clk_freq == 0) {
		pr_err(">>>>>>>>>>>>>>>>>> %s, cam_src_clk_freq = %d\n", __func__, cam_src_clk_freq);
		cam_src_clk_freq = clk_round_rate(ov5645->cam_src_clk, cam_src_clk_freq);
		pr_err(">>>>>>>>>>>>>>>>>> %s, cam_src_clk_freq = %d\n", __func__, cam_src_clk_freq);
		if (cam_src_clk_freq < 0) {
			dev_err(dev, "could not round cam_src_clk");
			return ret;
		}
		ret= clk_set_rate(ov5645->cam_src_clk,
					cam_src_clk_freq);
		if (ret) {
			dev_err(dev, "could not set cam_src_clk");
			return ret;
		}
	}
	...
	pr_err(">>>>>>>>>>>>>>>>>> %s, xclk_freq = %d\n", __func__, xclk_freq);

	ret = of_property_read_u32(dev->of_node, "clock-frequency", &xclk_freq);
	if (ret) {
		dev_err(dev, "could not get cam_clk frequency\n");
		return ret;
	}
	...
	xclk_freq = clk_round_rate(ov5645->xclk, xclk_freq);
	if (xclk_freq < 0) {
		dev_err(dev, "could not round xclk frequency\n");
		return ret;
	}
	pr_err(">>>>>>>>>>>>>>>>>> %s, xclk = %d\n", __func__, xclk_freq);
...
#if 0
	ov5645->mclk_gpio = devm_gpiod_get(dev, "mclk", GPIOD_OUT_HIGH);
	if (IS_ERR(ov5645->mclk_gpio)) {
		dev_err(dev, "cannot get mclk gpio\n");
		return PTR_ERR(ov5645->mclk_gpio);
	}
#endif
