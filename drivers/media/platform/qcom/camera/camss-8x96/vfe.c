...
#define VFE_0_GLOBAL_RESET_CMD		0x18 //0x00c
/*
...
*/
#define VFE_0_IRQ_CMD			0x58//0x024
#define VFE_0_IRQ_CMD_GLOBAL_CLEAR	(1 << 0)

#define VFE_0_IRQ_MASK_0		0x5C//0x028
...
#define VFE_0_IRQ_MASK_1		0x60//0x02c
...
#define VFE_0_IRQ_CLEAR_0		0x64//0x030
#define VFE_0_IRQ_CLEAR_1		0x68//0x034

#define VFE_0_IRQ_STATUS_0		0x6C//0x038
...
#define VFE_0_IRQ_STATUS_1		0x70//0x03c
...
#define VFE_0_IRQ_COMPOSITE_MASK_0	0x74//0x40
#define VFE_0_VIOLATION_STATUS		0x7C//0x48

#define VFE_0_BUS_CMD			0x80//0x4c
#define VFE_0_BUS_CMD_Mx_RLD_CMD(x)	(1 << (x))

#define VFE_0_BUS_CFG			0x84//0x050

#define VFE_0_BUS_XBAR_CFG_x(x)		(0x90 + 0x4 * ((x) / 2))//(0x58 + 0x4 * ((x) / 2))
...
#define VFE_0_BUS_IMAGE_MASTER_n_WR_CFG(n)		(0xA0 + (0x2C  * (n))) //(0x06c + 0x24 * (n))

#define VFE_0_BUS_IMAGE_MASTER_n_WR_CFG_WR_PATH_SHIFT	0
#define VFE_0_BUS_IMAGE_MASTER_n_WR_CFG_FRM_BASED_SHIFT	1
#define VFE_0_BUS_IMAGE_MASTER_n_WR_PING_ADDR(n)	(VFE_0_BUS_IMAGE_MASTER_n_WR_CFG(n) + 0x04)//(0x070 + 0x24 * (n))
#define VFE_0_BUS_IMAGE_MASTER_n_WR_PONG_ADDR(n)	(VFE_0_BUS_IMAGE_MASTER_n_WR_CFG(n) + 0x0C)// (0x074 + 0x24 * (n))
#define VFE_0_BUS_IMAGE_MASTER_n_WR_ADDR_CFG(n)	(VFE_0_BUS_IMAGE_MASTER_n_WR_CFG(n) + 0x14)	// (0x078 + 0x24 * (n))
#define VFE_0_BUS_IMAGE_MASTER_n_WR_ADDR_CFG_FRM_DROP_PER_SHIFT	2
#define VFE_0_BUS_IMAGE_MASTER_n_WR_ADDR_CFG_FRM_DROP_PER_MASK	(0x1F << 2)

#define VFE_0_BUS_IMAGE_MASTER_n_WR_UB_CFG(n)		(VFE_0_BUS_IMAGE_MASTER_n_WR_CFG(n) + 0x18)//(0x07c + 0x24 * (n))
...
#define VFE_0_BUS_IMAGE_MASTER_n_WR_FRAMEDROP_PATTERN(n)	(VFE_0_BUS_IMAGE_MASTER_n_WR_CFG(n) + 0x24)	//(0x088 + 0x24 * (n))
#define VFE_0_BUS_IMAGE_MASTER_n_WR_IRQ_SUBSAMPLE_PATTERN(n)	(VFE_0_BUS_IMAGE_MASTER_n_WR_CFG(n) + 0x28) //(0x08c + 0x24 * (n))
#define VFE_0_BUS_IMAGE_MASTER_n_WR_IRQ_SUBSAMPLE_PATTERN_DEF	0xffffffff

#define VFE_0_BUS_PING_PONG_STATUS	0x338//0x268

#define VFE_0_BUS_BDG_CMD		0x400//0x2c0
#define VFE_0_BUS_BDG_CMD_HALT_REQ	1

#define VFE_0_BUS_BDG_QOS_CFG_0		0x404//0x2c4
#define VFE_0_BUS_BDG_QOS_CFG_1		0x408//0x2c8
#define VFE_0_BUS_BDG_QOS_CFG_2		0x40c//0x2cc
#define VFE_0_BUS_BDG_QOS_CFG_3		0x410//0x2d0
#define VFE_0_BUS_BDG_QOS_CFG_4		0x414//0x2d4
#define VFE_0_BUS_BDG_QOS_CFG_5		0x418//0x2d8
#define VFE_0_BUS_BDG_QOS_CFG_6		0x41c//0x2dc
#define VFE_0_BUS_BDG_QOS_CFG_7		0x420//0x2e0

#define VFE_0_RDI_CFG_x(x)		(0x46C + (0x4 * (x)))//(0x2e8 + (0x4 * (x)))
...
*/
#define VFE_0_REG_UPDATE			0x4AC//0x378
...
*/
#define VFE_0_CGC_OVERRIDE_1			0x3C//0x974
...
{
	pr_err("[camera] : %s, val = 0x%x, reg = 0x%x\n",
				__func__, 1 << VFE_0_BUS_IMAGE_MASTER_n_WR_CFG_WR_PATH_SHIFT, VFE_0_BUS_IMAGE_MASTER_n_WR_CFG(wm));
...
static void vfe_wm_frame_based(struct vfe_device *vfe, u8 wm, u8 enable)
{
	if (enable) {
		...
		pr_err("[camera] : %s, val = 0x%x, reg = 0x%x + 0x14\n",
			__func__, 1 << VFE_0_BUS_IMAGE_MASTER_n_WR_CFG_FRM_BASED_SHIFT, VFE_0_BUS_IMAGE_MASTER_n_WR_CFG(wm));
		vfe_reg_set(vfe, VFE_0_BUS_IMAGE_MASTER_n_WR_CFG(wm) + 0x14,
			1 << VFE_0_BUS_IMAGE_MASTER_n_WR_CFG_FRM_BASED_SHIFT);
		vfe_reg_set(vfe, VFE_0_BUS_IMAGE_MASTER_n_WR_CFG(wm) + 0x20,
			1 << VFE_0_BUS_IMAGE_MASTER_n_WR_CFG_FRM_BASED_SHIFT);
	}
	else {
		vfe_reg_clr(vfe, VFE_0_BUS_IMAGE_MASTER_n_WR_CFG(wm) + 0x14,
			1 << VFE_0_BUS_IMAGE_MASTER_n_WR_CFG_FRM_BASED_SHIFT);
		vfe_reg_clr(vfe, VFE_0_BUS_IMAGE_MASTER_n_WR_CFG(wm) + 0x20,
			1 << VFE_0_BUS_IMAGE_MASTER_n_WR_CFG_FRM_BASED_SHIFT);
	}
}
#if 0
			      ...
			      u16 depth)
{
	...
	pr_err("[camera] : %s, val = 0x%x, reg = 0x%x\n", __func__, reg, VFE_0_BUS_IMAGE_MASTER_n_WR_UB_CFG(wm));
...
static void vfe_bus_reload_wm(struct vfe_device *vfe, u8 wm)
{
	...
	pr_err("[camera] : %s, val = 0x%x, reg = 0x%x\n", __func__, VFE_0_BUS_CMD_Mx_RLD_CMD(wm), VFE_0_BUS_CMD);
	wmb();
...
{
	if (enable)
		writel_relaxed(0x00000101, vfe->base + VFE_0_BUS_CFG);
	else
				      ...
				      enum vfe_line_id id)
{
	...
	pr_err("[camera] : %s, val = 0x%x, reg = 0x%x\n", __func__, reg, VFE_0_RDI_CFG_x(0));
	vfe_reg_set(vfe, VFE_0_RDI_CFG_x(0), reg);

	reg = VFE_0_RDI_CFG_x_RDI_EN_BIT;
	reg |= ((3 * id) << VFE_0_RDI_CFG_x_RDI_STREAM_SEL_SHIFT) &
		VFE_0_RDI_CFG_x_RDI_STREAM_SEL_MASK;
	pr_err("[camera] : %s, val = 0x%x, reg = 0x%x\n", __func__, reg, VFE_0_RDI_CFG_x(id));
	...
	reg = 0xC00; //mvali TODO
	pr_err("[camera] : %s, val = 0x%x, reg = 0x%x\n", __func__, reg, VFE_0_BUS_XBAR_CFG_x(wm));
...
{
	pr_err("[camera] : %s, val = 0x%x, reg = 0x%x\n",
		__func__, VFE_0_BUS_IMAGE_MASTER_n_WR_IRQ_SUBSAMPLE_PATTERN_DEF, VFE_0_BUS_IMAGE_MASTER_n_WR_IRQ_SUBSAMPLE_PATTERN(wm));
...
static void vfe_set_rdi_cid(struct vfe_device *vfe, enum vfe_line_id id, u8 cid)
{
	...
	pr_err("[camera] : %s, val = 0x%x, reg = 0x%x\n", __func__, cid << VFE_0_RDI_CFG_x_RDI_M0_SEL_SHIFT, VFE_0_RDI_CFG_x(id));
...
//		      VFE_0_IRQ_MASK_1_RDIn_SOF(line_id);
	...
	u32 irq_en0 = 0xE00000F3;
	u32 irq_en1 = 0xFFFFFFFF;

	if (enable) {
		pr_err("[camera] : %s, irq_en0 = 0x%x, reg = 0x%x\n", __func__, irq_en0, VFE_0_IRQ_MASK_0);
		vfe_reg_set(vfe, VFE_0_IRQ_MASK_0, irq_en0);
		pr_err("[camera] : %s, irq_en1 = 0x%x, reg = 0x%x\n", __func__, irq_en1, VFE_0_IRQ_MASK_1);
...
{
	u32 val = 0xcccc0011;
	u32 val17 = 0x40000103;

	writel_relaxed(val, vfe->base + 0x424);
	writel_relaxed(val, vfe->base + 0x428);
	writel_relaxed(val, vfe->base + 0x42C);
	writel_relaxed(val, vfe->base + 0x430);
	writel_relaxed(val, vfe->base + 0x434);
	writel_relaxed(val, vfe->base + 0x438);
	writel_relaxed(val, vfe->base + 0x43C);
	writel_relaxed(val, vfe->base + 0x440);
	writel_relaxed(val, vfe->base + 0x444);
	writel_relaxed(val, vfe->base + 0x448);
	writel_relaxed(val, vfe->base + 0x44C);
	writel_relaxed(val, vfe->base + 0x450);
	writel_relaxed(val, vfe->base + 0x454);
	writel_relaxed(val, vfe->base + 0x458);
	writel_relaxed(val, vfe->base + 0x45C);
	writel_relaxed(val, vfe->base + 0x460);
	writel_relaxed(val17, vfe->base + 0x464);
}


static void vfe_set_vbif(struct vfe_device *vfe)
{
	writel_relaxed(0x3, vfe->vbif_base + 0x124);
}
...
static void vfe_set_cgc_override(struct vfe_device *vfe, u8 wm, u8 enable)
{
	...
	pr_err("[camera] : %s, val = 0x%x, reg = 0x%x\n", __func__, val, VFE_0_CGC_OVERRIDE_1);
	if (enable)
...
//	unsigned int i;
	u16 ub_size;
	pr_err("[camera] : %s, name = %s\n", __func__, line->subdev.name);
	switch (vfe->id) {
	case 0:
	...
	if (line->id != VFE_LINE_PIX) {
		vfe_set_cgc_override(vfe, output->wm_idx[0], 1); //msm_vfe47_axi_update_cgc_override
		vfe_enable_irq_wm_line(vfe, output->wm_idx[0], line->id, 1); //msm_vfe47_axi_cfg_wm_irq_mask
		vfe_bus_connect_wm_to_rdi(vfe, output->wm_idx[0], line->id); //msm_vfe47_cfg_rdi_reg, msm_vfe47_axi_cfg_wm_xbar_reg
		vfe_wm_set_subsample(vfe, output->wm_idx[0]); //msm_vfe47_axi_cfg_wm_reg   ------ TODO
		vfe_set_rdi_cid(vfe, line->id, 0); // msm_vfe47_cfg_rdi_reg --------- TODO
		vfe_wm_set_ub_cfg(vfe, output->wm_idx[0],
				  (ub_size + 1) * output->wm_idx[0], ub_size); //msm_vfe47_cfg_axi_ub_equal_default
		vfe_wm_frame_based(vfe, output->wm_idx[0], 1);
		vfe_wm_enable(vfe, output->wm_idx[0], 1); // msm_vfe47_axi_enable_wm
		vfe_bus_reload_wm(vfe, output->wm_idx[0]); //msm_vfe47_axi_reload_wm
	} else {
		pr_err("[camera] : %s, VFE_LINE_PIX", __func__);
#if 0
...
//	dev_err_ratelimited(to_device(vfe), "=============== %s, 0x%x, 0x%x\n", __func__, value0, value1);
	...
	if (value1 & VFE_0_IRQ_STATUS_1_BUS_BDG_HALT_ACK) {
		dev_err_ratelimited(to_device(vfe), "=============== %s, %d\n", __func__, __LINE__);
...
static int vfe_get(struct vfe_device *vfe)
{
	...
	if (vfe->power_count == 0) {
		ret = regulator_enable(vfe->vdd);
		if (ret < 0){
			dev_err(to_device(vfe), "Can not enable vdd regulator\n");
			return ret;
		}
		ret = regulator_enable(vfe->gdscr);
		if (ret < 0){
			dev_err(to_device(vfe), "Can not enable camss regulator\n");
			return ret;
		}
		ret = regulator_enable(vfe->mmagic);
		if (ret < 0){
			dev_err(to_device(vfe), "Can not enable mmagic regulator\n");
			return ret;
		}
...
error_reset:
	...
	regulator_disable(vfe->vdd);
	regulator_disable(vfe->gdscr);
	regulator_disable(vfe->mmagic);
...
static int vfe_set_stream(struct v4l2_subdev *sd, int enable)
{
	...
	struct vfe_device *vfe = to_vfe(line);
	int ret;
	pr_err("[camera] : %s, name = %s, enable = %d\n", __func__, sd->name, enable);
	if (enable) {
		ret = vfe_enable(line);
		if (ret < 0)
			   ...
			   enum v4l2_subdev_format_whence which)
{
	...
	case MSM_VFE_PAD_SRC:
		...
		*fmt = *__vfe_get_format(line, fh, MSM_VFE_PAD_SINK,
					 which);
			      ...
			      struct v4l2_subdev_mbus_code_enum *code)
{
	...
	if (code->pad == MSM_VFE_PAD_SINK) {
		...
		format = __vfe_get_format(line, fh, MSM_VFE_PAD_SINK,
					  code->which);
			       ...
			       struct v4l2_subdev_frame_size_enum *fse)
{
	...
	format.width = 1;
	format.height = 1;
	vfe_try_format(line, fh, fse->pad, &format, fse->which);
			  ...
			  struct v4l2_subdev_format *fmt)
{
	struct vfe_line *line = v4l2_get_subdevdata(sd);
	struct v4l2_mbus_framefmt *format;
//	pr_err("[camera] : %s, name = %s, line = %d\n", __func__, sd->name, __LINE__);
	format = __vfe_get_format(line, fh, fmt->pad, fmt->which);
	if (format == NULL)
		return -EINVAL;
			  ...
			  struct v4l2_subdev_format *fmt)
{
	...
	vfe_try_format(line, fh, fmt->pad, &fmt->format, fmt->which);
	*format = fmt->format;

	/* Propagate the format from sink to source */
	if (fmt->pad == MSM_VFE_PAD_SINK) {
		format = __vfe_get_format(line, fh, MSM_VFE_PAD_SRC,
					  fmt->which);

		*format = fmt->format;
		vfe_try_format(line, fh, MSM_VFE_PAD_SRC, format,
			       fmt->which);
	}
...
static int vfe_init_formats(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	return vfe_set_format(sd, fh ? fh : NULL, &format);
}
...
int msm_vfe_subdev_init(struct vfe_device *vfe, struct resources *res)
{
	...
	vfe->vbif_base = devm_ioremap_resource(dev, r);
	...
	/*regulators/gdscs */
	vfe->vdd = devm_regulator_get(dev, res->regulator[0]);
	if (IS_ERR(vfe->vdd)) {
		dev_err(dev, "could not get regulator\n");
		return PTR_ERR(vfe->vdd);
	}
	vfe->gdscr = devm_regulator_get(dev, res->regulator[1]);
	if (IS_ERR(vfe->gdscr)) {
		dev_err(dev, "could not get regulator\n");
		return PTR_ERR(vfe->gdscr);
	}
	vfe->mmagic = devm_regulator_get(dev, res->regulator[2]);
	if (IS_ERR(vfe->mmagic)) {
		dev_err(dev, "could not get regulator\n");
		return PTR_ERR(vfe->mmagic);
	}
			      ...
			      struct v4l2_device *v4l2_dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(vfe->line); i++) {
		...
		ret = media_entity_init(&sd->entity, MSM_VFE_PADS_NUM,
					     pads, 0);
		if (ret < 0) {
