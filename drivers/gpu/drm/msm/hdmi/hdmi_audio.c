        ...
        u32 level_shift, bool down_mix)*/
{
        ...
        if (hdmi->hdmi_mode && hdmi->power_on) {
                ...
                if (params->sample_rate_hz == 32000)
                        params->sample_rate_hz = MSM_HDMI_SAMPLE_RATE_32KHZ;
                else if (params->sample_rate_hz == 44100)
                        params->sample_rate_hz = MSM_HDMI_SAMPLE_RATE_44_1KHZ;
                else if (params->sample_rate_hz == 48000)
                        params->sample_rate_hz = MSM_HDMI_SAMPLE_RATE_48KHZ;
                else if (params->sample_rate_hz == 88200)
                        params->sample_rate_hz = MSM_HDMI_SAMPLE_RATE_88_2KHZ;
                else if (params->sample_rate_hz == 96000)
                        params->sample_rate_hz = MSM_HDMI_SAMPLE_RATE_96KHZ;
                else if (params->sample_rate_hz == 176400)
                        params->sample_rate_hz = MSM_HDMI_SAMPLE_RATE_176_4KHZ;
                else if (params->sample_rate_hz == 192000)
                        params->sample_rate_hz = MSM_HDMI_SAMPLE_RATE_192KHZ;

                audio->rate = params->sample_rate_hz;
                info->channels = params->num_of_channels;
                info->channel_allocation = params->channel_allocation;
                info->level_shift_value = params->level_shift;
                info->downmix_inhibit = params->down_mix;
