...
static int msm_camera_pinctrl_init(struct ov5640 *ov5640)
{
	struct pinctrl_info *sensor_pctrl = NULL;

	sensor_pctrl = &ov5640->pinctrl_info;
	sensor_pctrl->pinctrl = devm_pinctrl_get(ov5640->dev);
...
static int ov5640_set_power_on(struct ov5640 *ov5640)
{
	...
	ret = msm_camera_pinctrl_init(ov5640);
	if (ret < 0) {
		pr_err("%s:%d Initialization of pinctrl failed\n",
				__func__, __LINE__);
		ov5640->cam_pinctrl_status = 0;
	} else {
		ov5640->cam_pinctrl_status = 1;
	}
	if (ov5640->cam_pinctrl_status) {
		ret = pinctrl_select_state(ov5640->pinctrl_info.pinctrl,
			ov5640->pinctrl_info.gpio_state_active);
		if (ret)
			pr_err("%s:%d cannot set pin to active state",
				__func__, __LINE__);
	}
	ret = clk_prepare_enable(ov5640->cam_src_clk);
	if (ret < 0) {
		dev_err(ov5640->dev, "cam_src_clk prepare enable failed\n");
		ov5640_regulators_disable(ov5640);
		return ret;
	}
	ret = clk_prepare_enable(ov5640->xclk);
	if (ret < 0) {
		dev_err(ov5640->dev, "xclk prepare enable failed\n");
		ov5640_regulators_disable(ov5640);
		return ret;
	}
...
if (ov5640->cam_pinctrl_status) {
		ret = pinctrl_select_state(ov5640->pinctrl_info.pinctrl,
				ov5640->pinctrl_info.gpio_state_suspend);
		if (ret)
			pr_err("%s:%d cannot set pin to suspend state",
				__func__, __LINE__);
		devm_pinctrl_put(ov5640->pinctrl_info.pinctrl);
		ov5640->cam_pinctrl_status = 0;
	}
	ov5640_regulators_disable(ov5640);
...
__ov5640_get_pad_crop(struct ov5640 *ov5640, struct v4l2_subdev_fh *fh,
			     ...
			     struct v4l2_subdev_format *format)
{
	...
	__crop = __ov5640_get_pad_crop(ov5640, fh, format->pad,
			format->which);
			   ...
			   struct v4l2_subdev_selection *sel)
{
	...
	sel->r = *__ov5640_get_pad_crop(ov5640, fh, sel->pad,
					sel->which);
	return 0;
}
...
//	ov5640->fmt.colorspace = V4L2_COLORSPACE_JPEG;
	ov5640->fmt.colorspace = V4L2_COLORSPACE_SRGB;
	...
	of_node_put(endpoint);
	ov5640->cam_src_clk = devm_clk_get(dev, "cam_src_clk");
	if (IS_ERR(ov5640->cam_src_clk)) {
		dev_err(dev, "could not get cam_src_clk");
		return PTR_ERR(ov5640->cam_src_clk);
	}

	cam_src_clk_freq = clk_get_rate(ov5640->cam_src_clk);
	if (cam_src_clk_freq == 0) {
		pr_err(">>>>>>>>>>>>>>>>>> %s, cam_src_clk_freq = %d\n", __func__, cam_src_clk_freq);
		cam_src_clk_freq = clk_round_rate(ov5640->cam_src_clk, cam_src_clk_freq);
		pr_err(">>>>>>>>>>>>>>>>>> %s, cam_src_clk_freq = %d\n", __func__, cam_src_clk_freq);
		if (cam_src_clk_freq < 0) {
			dev_err(dev, "could not round cam_src_clk");
			return ret;
		}
		ret= clk_set_rate(ov5640->cam_src_clk,
					cam_src_clk_freq);
		if (ret) {
	...
	xclk_freq = clk_get_rate(ov5640->xclk);
	...
	xclk_freq = clk_round_rate(ov5640->xclk, xclk_freq);
	if (xclk_freq < 0) {
